...
namespace Memcury
{
    ...
    {
        template <typename T>
        inline constexpr static auto IsInRange(T value, T min, T max) -> bool
        {
            return value >= min && value < max;
        }

        inline constexpr auto StrHash(const char* str, int h = 0) -> unsigned int
        {
    ...
    namespace Safety
    {
        ...
        inline static auto FreezeCurrentThread() -> void
        {
            SuspendThread(GetCurrentThread());
        }

        inline static auto PrintStack(CONTEXT* ctx) -> void
        {
            STACKFRAME64 stack;
        ...
        inline auto MemcuryGlobalHandler(EXCEPTION_POINTERS* ExceptionInfo) -> long
        {
        ...
        template <ExceptionMode mode>
        inline static auto SetExceptionMode() -> void
        {
    ...
    namespace ASM
    {
        ...
        inline constexpr auto MnemonicToString(MNEMONIC e) -> const char*
        {
            switch (e)
            {
            case JMP_REL8:
        ...
        inline constexpr auto Mnemonic(const char* s) -> MNEMONIC
        {
        ...
        inline static auto pattern2bytes(const char* pattern) -> std::vector<int>
        {
    ...
    namespace PE
    {
        ...
        public:
            ...
            inline auto operator=(uintptr_t address) -> Address
            {
                _address = address;
                return *this;
            }

            inline auto operator=(void* address) -> Address
            {
                _address = reinterpret_cast<uintptr_t>(address);
                return *this;
            }

            inline auto operator+(uintptr_t offset) -> Address
            {
                return Address(_address + offset);
            }

            inline bool operator>(uintptr_t offset)
            {
            ...
            inline  auto RelativeOffset(uint32_t offset) -> Address
            {
                _address = ((_address + offset + 4) + *(int32_t*)(_address + offset));
                return *this;
            }

            inline auto AbsoluteOffset(uint32_t offset) -> Address
            {
                _address = _address + offset;
                return *this;
            }

            inline auto Get() -> uintptr_t
            {
                return _address;
            }
        ...
        public:
            ...
            inline static auto GetAllSections() -> std::vector<Section>
            {
            ...
            inline static auto GetSection(std::string sectionName) -> Section
            {
            ...
            inline  auto GetSectionSize() -> uint32_t
            {
                return rawSection.Misc.VirtualSize;
            }

            inline auto GetSectionStart() -> Address
            {
                return Address(GetModuleBase() + rawSection.VirtualAddress);
            }

            inline auto GetSectionEnd() -> Address
            {
                return Address(GetSectionStart() + GetSectionSize());
            }

            inline  auto isInSection(Address address) -> bool
            {
    ...
    public:
        ...
        inline static auto SetTargetModule(const char* moduleName) -> void
        {
            PE::SetCurrentModule(moduleName);
        }

        inline  static auto FindPatternEx(HANDLE handle, const char* pattern, const char* mask, uint64_t begin, uint64_t end) -> Scanner
        {
        ...
        inline static auto FindPatternEx(HANDLE handle, const char* sig) -> Scanner
        {
            char pattern[100];
            char mask[100];
        ...
        inline static auto FindPattern(const char* signature) -> Scanner
        {
        ...
        inline static auto FindStringRef(T string) -> Scanner
        {
            PE::Address add{ nullptr };
        ...
        inline auto FindFunctionBoundary(bool forward = false) -> Scanner
        {
            const auto scanBytes = _address.GetAs<std::uint8_t*>();
        ...
        inline auto RelativeOffset(uint32_t offset) -> Scanner
        {
        ...
        inline auto AbsoluteOffset(uint32_t offset) -> Scanner
        {
    ...
    class TrampolineHook
    {
        ...
        inline void* AllocatePageNearAddress(void* targetAddr)
        {
            SYSTEM_INFO sysInfo;
        ...
        inline void WriteAbsoluteJump(void* jumpLocation, void* destination)
        {
        ...
        inline uintptr_t PrepareRestore()
        {
            /*
        ...
        inline auto PrepareJMPInstruction(uint64_t dst)
        {
